...
- Yifei Wang, Dizhan Xue, Shengjie Zhang, and Shengsheng Qian, "BadAgent:
  Inserting and Activating Backdoor Attacks in LLM Agents," ACL 2024.
  Main reading target of this note. The important shift is from content-level
  backdoors to action-level backdoors.
- Project repository: `https://github.com/DPamK/BadAgent`.
...
- Wenkai Yang, Xiaohan Bi, Yankai Lin, Sishuo Chen, Jie Zhou, and Xu Sun,
  ...
  Agents," NeurIPS 2024.
  Good companion paper. It separates agent backdoors by trigger location and by
  whether the malicious behavior appears in the final answer or only in the
  intermediate reasoning/action trace.
- Yunhao Feng et al., "BackdoorAgent: A Unified Framework for Backdoor Attacks
  on LLM-based Agents," arXiv:2601.04566, 2026.
  Useful follow-up after BadAgent: it frames the agent attack surface around
  planning, memory, and tool-use stages, then studies trigger propagation across
  the whole workflow.

### LLM Backdoor Background

- Yige Li, Hanxun Huang, Yunhan Zhao, Xingjun Ma, and Jun Sun, "BackdoorLLM:
  A Comprehensive Benchmark for Backdoor Attacks and Defenses on Large Language
  Models," arXiv:2408.12798, 2024.
  Benchmark-style background for LLM backdoor mechanisms: data poisoning,
  weight poisoning, hidden-state attacks, and CoT-style attacks.
- Evan Hubinger et al., "Sleeper Agents: Training Deceptive LLMs that Persist
  Through Safety Training," arXiv:2401.05566, 2024.
  Not an agent tool paper in the same sense, but important for the persistence
  question: standard safety training may fail to remove conditional deceptive
  behavior.
- Yuanpu Cao, Bochuan Cao, and Jinghui Chen, "Stealthy and Persistent
  Unalignment on Large Language Models via Backdoor Injections," NAACL 2024.
  Closely related to the "clean-data fine-tuning is insufficient" observation.
- Zhen Xiang, Fengqing Jiang, Zidi Xiong, Bhaskar Ramasubramanian,
  Radha Poovendran, and Bo Li, "BadChain: Backdoor Chain-of-Thought Prompting
  for Large Language Models," ICLR 2024.
  Useful for comparing training-time backdoors with in-context demonstration
  poisoning and CoT trigger paths.

### Environment, Retrieval, and Memory Poisoning

- Wei Zou, Runpeng Geng, Binghui Wang, and Jinyuan Jia, "PoisonedRAG: Knowledge
  Corruption Attacks to Retrieval-Augmented Generation of Large Language
  Models," USENIX Security 2025.
  Relevant to passive BadAgent-style triggers: the attacker does not need to
  touch the user query if the external knowledge base or memory store can be
  poisoned.
- Qiusi Zhan, Zhixiang Liang, Zifan Ying, and Daniel Kang, "InjecAgent:
  ...
  Model Agents," ACL Findings 2024.
  Focuses on indirect prompt injection in tool-integrated agents; useful for
  comparing training-time backdoors with inference-time environment attacks.

### Agent Security Benchmarks and Defenses

- Edoardo Debenedetti et al., "AgentDojo: A Dynamic Environment to Evaluate
  Prompt Injection Attacks and Defenses for LLM Agents," NeurIPS 2024.
  A benchmark for studying utility/security trade-offs in tool-calling agents.
- Xuchen Suo, "Signed-Prompt: A New Approach to Prevent Prompt Injection
  Attacks Against LLM-Integrated Applications," arXiv:2401.07612, 2024.
  Worth reading for the defense idea that instruction provenance should be
  explicit rather than inferred from raw text.
...
- Xiang Deng, Yu Gu, Boyuan Zheng, Shijie Chen, Samuel Stevens, Boshi Wang,
  ...
  NeurIPS 2023.
  The web-navigation benchmark used in many agent studies.
- Shunyu Yao et al., "WebShop: Towards Scalable Real-World Web Interaction
  with Grounded Language Agents," NeurIPS 2022.
  Useful background for the WebShop task and shopping-agent attack surface.
